...
void EEPROM_write(unsigned char EEPROM_dat, unsigned char EEPROM_AddrH,
		unsigned char EEPROM_AddrL)
{
	unsigned char i;
...
unsigned char EEPROM_Read(unsigned char EEPROM_AddrH,
		unsigned char EEPROM_AddrL)
{
	unsigned char rs, i;
...
unsigned int time_str_to_int(unsigned char time_str[])
{
	...
	if ((time_str[0] < '0' || time_str[0] > '2') || (time_str[1] < '0'
			|| time_str[1] > '9') || (time_str[2] < '0' || time_str[2] > '5')
...
void write_all_data(unsigned char cnt, unsigned char index,
		HistoryData* his_data, unsigned int data_addr)
{
	unsigned int i;
	...
	read_data_array(sys_data, SYSTEM_DATA_SIZE, SYSTEM_DATA_START_ADDR
			+ HISTORY_DATA_BACKUP_START_ADDR);
...
void get_sn_data()
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		{
			flag = 1;
			system_data[PRICE_OFFSET + i] = price_per_area[i + 1];
		}
