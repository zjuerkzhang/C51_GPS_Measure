	...
	while (1)
	{
		...
		if (GPS_UPDATA == 1)
		{
			...
			if ((1 == g_page_id) && (1 == celiang_mode) && (signal))
			{
				...
				if (1 == danwei_sel) //公顷
					sprintf(g_area_value, "%08.1f", GeodeticGetArea()/10000+0.05);
				else //默认面积单位为亩
					sprintf(g_area_value, "%08.1f", GeodeticGetArea()/666.666667+0.05);

				if (3 == danwei_sel) //千米
					sprintf(g_length_value, "%08.1f", GeodeticGetDistance()/1000+0.05);
				else // 默认长度单位为米
